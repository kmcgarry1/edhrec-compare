<template>
  ...
  >
    <div class="space-y-2">
      <div class="flex flex-wrap items-center gap-2">
        <CBadge tone="muted" variant="outline" size="sm"> Utility panel </CBadge>
        <CBadge
      ...
      <h2 class="text-lg font-semibold text-[color:var(--text)]">Inventory and filters</h2>
    ...
    <CNotice
      ...
      :message="
        hasCsvData
          ? 'Collection matching is active across the current commander route.'
          : 'Upload your collection to unlock owned and unowned cardlist views.'
      "
      :title="hasCsvData ? 'CSV ready' : 'CSV pending'"
      :class="hasCsvData ? '' : 'border-dashed bg-[color:var(--surface-muted)]'"
    />

    <CButton type="button" variant="primary" class="w-full" @click="emit('open-upload')">
    ...
    <CSurface variant="content" size="sm" class="space-y-3">
      ...
      <div class="grid grid-cols-3 gap-2 text-[0.72rem] font-semibold">
        <button
          ...
          :key="option.label"
          type="button"
          class="rounded border px-2 py-2 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--accent)]"
  ...
  <Card variant="utility" padding="p-0" class="overflow-hidden">
    <details class="group">
      <summary class="cursor-pointer px-4 py-3 text-sm font-semibold text-[color:var(--text)]">
        Top 50 scan
      </summary>
      <div class="px-4 pb-4">
