...
```html
<div class="grid">
  Bracket: [All ▼] Budget: [Any ▼] Page Type: [Commander ▼] Companion: [None ▼]
</div>
```
...
<div class="lg:flex lg:gap-6">
  <aside class="lg:w-64 lg:sticky lg:top-4">
    <!-- Filters -->
  </aside>
  <main class="lg:flex-1 lg:grid lg:grid-cols-2 lg:gap-4">
    <!-- Cards -->
  </main>
</div>
```
...
const { density, spacing } = useLayoutDensity();
const densityClasses = computed(() => `density-${density.value}`);
</script>
```
...
- #70 - Visual hierarchy (affects toolbar density)
- #69 - Mobile toolbar (density implications for mobile)
- #72 - Empty states (affects perceived density)
